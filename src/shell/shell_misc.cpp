...
void DOS_Shell::ProcessCmdLineEnvVarStitution(char *line)
{
	...
	/* if the incoming string is already too long, then that's a problem
	 * too! */
	...
	while (*r != 0) {
		if (*r == '%') {
			r++;
			if (*r == '%' || *r == 0) {
				/* %% or leaving a trailing % at the end (Win95
				 * behavior) becomes a single '%' */
				if (w >= wf)
					goto overflow;
				*w++ = '%';
				if (*r != 0)
				...
				const char *name = r; /* store pointer, 'r' is
				...
				if (*r == '%' && ((spaces > 0 && chars == 0) ||
					...
					*r++ = 0; /* ASCIIZ snip */
					...
					while (*r == ' ')
						r++; /* skip spaces */
					name--; /* step "name" back to cover the
					           first '%' we found */
	...
	*w = 0;
	...
	{
		size_t out_len = (size_t)(w + 1 - temp); /* length counting the
		                                            NUL too */
